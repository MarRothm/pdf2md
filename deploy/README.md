...
| A document that used to convert is refused at upload as "damaged" | The structural reader in the web image became stricter — its limits are not the converter's | `web` logs carry `upload_rejected … detail="…"` with the reader's own words. A page tree deeper than 100 levels, which merging many PDFs produces, was refused this way by pypdf 6.16.2; the library is pinned exactly for this reason and the ceiling is raised in `pdfinfo` |
