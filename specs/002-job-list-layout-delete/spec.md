...
- **FR-022**: The system MUST refuse to delete a document while a conversion of it is with the converter, including conversions the operator is not currently looking at, so that work in flight cannot write output back after the deletion. A conversion that is merely *waiting* MUST NOT block deletion: it has never been handed to the converter, so removing its entry is what takes it out of the queue. Blocking on waiting made an entry the system never picked up impossible to remove at all.
