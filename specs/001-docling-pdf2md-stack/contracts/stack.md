...
| `PDF2MD_IMAGE_HEADER_BAND` | `0.15` | A picture lying entirely within this fraction of the page from the top is the page's furniture — a party logo — and is not extracted (feature 003 FR-014). Position separates furniture from figures where size never could: the same logo appears at many sizes |
| `PDF2MD_IMAGE_FOOTER_BAND` | `0.10` | The same, from the bottom. **Measured** on the corpus: the logo's lower edge is at 10.7% of page height and the body begins at 14.6%, so 0.15 leaves 13 mm of margin on A4 without being able to swallow a figure — furniture must lie *entirely* inside the band |
